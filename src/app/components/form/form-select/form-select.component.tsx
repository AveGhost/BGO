...
const FormSelect = ({isOpen,onClick,icon,title,elements,singleSelect,multiSelect}: FormSelectProps) => {
    return (
        <div className="w-full relative py-2 px-4 border-1 border-zinc-600 rounded-lg text-zinc-400" onClick={onClick}>
            <span className={`cursor-pointer w-full block ${title !== '' ? 'text-zinc-300' : ''}`}>{title ? title : 'Wybierz platforme do recenzji:'}
