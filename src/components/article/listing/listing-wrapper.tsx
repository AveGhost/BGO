    ...
    return (
        <div className={`grid ${isGrid ? "xl:grid-cols-4 md:grid-cols-2 sm:grid-cols-1" : "grid-cols-1"} gap-8 py-8 px-4 container mx-auto`}>
            ...
            {posts && posts.content.map((item) => (
                <ArticleCard key={item.id} href={`/article/${item.id}`} rate={item.score} isGrid={isGrid} image={item.thumbnail} title={item.title} description={item.teaser} author={{name: item.author.firstName, date: item.publishDate}}/>
            ))}
        </div>
    )
}
