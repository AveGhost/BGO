        ...
        <div className={`mobile-menu fixed ${isMobileMenuOpen ? "right-0 opacity-100" : "-right-100 opacity-0"} top-0 w-full max-w-[300px] h-full bg-zinc-800 flex flex-col z-50 p-4 shadow-lg shadow-zinc-600 transition-opacity duration-300`}>
            ...
            <>
                <ul className="mt-6 border-t border-zinc-600 py-2 w-full">
                    {user.userRole === Roles.EDITOR || user.userRole === Roles.ADMINISTRATOR ? <DropDownElement url="/dashboard/add-game" name="Dodaj recencje" icon="ic:round-post-add" onClick={toggleMobileMenu} /> : null}
                    {user.userRole === Roles.EDITOR || user.userRole === Roles.ADMINISTRATOR ? <DropDownElement url="/dashboard/my-posts" name="Twoje posty" icon="dashicons:admin-post" onClick={toggleMobileMenu} /> : null}
                    {user.userRole === Roles.ADMINISTRATOR && <DropDownElement url="/" name="Wszystkie posty" icon="ic:round-all-inbox" onClick={toggleMobileMenu} />}
